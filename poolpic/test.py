...
red_channel=ndr[0][:,:,2]
green_channel=ndr[0][:,:,1]
blue_channel=ndr[0][:,:,0]

my_reconstructed_image=cv2.merge([red_channel, green_channel, blue_channel, red_channel])

fig, (ax1, ax2) = plt.subplots(1,2,figsize=(12,5))
ax1.imshow(my_reconstructed_image)#ndr[0])#[:,:,0])
ax1.axis('off')
ax2.axis('off')
#ax2.imshow(obq[0])
plt.show()
